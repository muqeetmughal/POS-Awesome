<template>
  <div>
    ...
    <v-card flat class="cards mb-0 mt-3 py-0">
      <v-row align="start" no-gutters>
        ...
        <v-col cols="12" v-if="is_slipy_enabled">
          <v-btn block large class="mt-2 pa-1" color="info" dark @click="submitWithSlipy">
            {{ __("Go Slipy") }}
          </v-btn>
        </v-col>
        <v-col cols="12">
    ...
    <div>
      ...
      <v-dialog v-model="slipy_dialog" max-width="400px">
        <v-card>
          <v-card-title>
            <span class="headline primary--text">{{
              __("Enter Slipy id")
          ...
          <v-card-text class="pa-0">
            <v-container>
              <v-text-field dense outlined color="primary" :label="frappe._('Slipy ID')" background-color="white"
                hide-details v-model="slipy_id" type="text"></v-text-field>
            </v-container>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" dark @click="slipy_dialog = false">{{
              __("Close")
              }}</v-btn>
            <v-btn color="primary" dark @click="submitSlipyInvoice">{{
              __("Submit")
              }}</v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    submitSlipyInvoice() {
      ...
      this.slipy_dialog = false
    },
    submitWithSlipy() {
      this.submitted_with_slipy = true;
      this.slipy_dialog = true

    },
    checkSlipyEnabled() {
      frappe.call({
        method: "slipy.slipy.api.slipy.is_slipy_enabled",
        callback: function (r) {
          if (r) {
            this.is_slipy_enabled = r.message === 1;
          }
        }.bind(this),
      });
    },
        ...
        callback: function (r) {
          if (r.message) {
            ...
            if (vm.submitted_with_slipy) {
              vm.create_slipy_invoice(vm.invoice_doc.name, vm.slipy_id)
              evntBus.$emit("show_mesage", {
